...
import { PieChart, Upload, Play } from "lucide-react"
...
export function PieChartAnalyzerForm() {
  ...
  const parseCSV = (text: string) => {
    ...
    return rows.map((line) => {
      ...
      return { name: name.trim(), value: Number(value.trim()) }
    })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setResult(`Analyzed pie chart data from ${file ? file.name : "manual input"}`)
    }, 2000)
  }

  const pieChartData = useMemo(() => {
    const labels = parsedData.map((d) => d.name)
  ...
  return (
    <AIFormLayout
      ...
      icon={<PieChart className="h-5 w-5 text-matrix" />}
    >
